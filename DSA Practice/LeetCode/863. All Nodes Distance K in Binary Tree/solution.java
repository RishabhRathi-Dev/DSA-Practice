...
class Solution {
    ...
    public List<Integer> distanceK(TreeNode root, TreeNode target, int K) {
        ...
        cloneGraph(root, null, target); // Creates graph of tree
        ...
        while (!q.isEmpty()) {
            ...
            int size = q.size(); // Level Size think of it as radial like in graph
            ...
            for (int i = 0; i < size; i++) {
                ...
                { 
                    visited.add(gNode.left); q.add(gNode.left); 
                }
                
                if (gNode.right != null && !visited.contains(gNode.right)) { 
                    visited.add(gNode.right); q.add(gNode.right); 
                }

                if (gNode.parent != null && !visited.contains(gNode.parent)) {
                    visited.add(gNode.parent); q.add(gNode.parent); 
                }
